Feature: check SMAPI UI Custom

  Background: User Opens App and Capture Events
    When Capture IOS RN Event
    When Click On IOS RN Button ["UI Custom"]
    When Capture IOS RN Event

  Scenario: Check UI Custom 1st events
  ...
  Scenario: Check UI Custom 2nd events
  ...
  Scenario: Check UI Custom 3rd events
...
#//////////////////////////////// Check Press me UI Custom

  Scenario: Check Navigate from UI Controls to Main Page 1st events
    When Click On IOS RN Button ["Press me"]
    When Capture IOS RN Event
    Then Validate IOS RN Element ["page-name" "UITracking"] for event ["UICustom" "CustomButton - CustomButtonId"]
    Then Validate IOS RN Element ["subpage-name" "NA"] for event ["UICustom" "CustomButton - CustomButtonId"]
    Then Validate IOS RN Element ["event-description" "onPress"] for event ["UICustom" "CustomButton - CustomButtonId"]
    Then Validate IOS RN Element ["x-vf-trace-timestamp" "SMAPI_Presence"] for event ["UICustom" "CustomButton - CustomButtonId"]
    Then Validate IOS RN Element ["device-orientation" "SMAPI_Presence"] for event ["UICustom" "CustomButton - CustomButtonId"]
    Then Validate IOS RN Element ["x-vf-user-id" "SMAPI_Presence"] for event ["UICustom" "CustomButton - CustomButtonId"]
    Then Validate IOS RN Element ["x-vf-trace-session-id" "SMAPI_Presence"] for event ["UICustom" "CustomButton - CustomButtonId"]
    Then Validate IOS RN Element ["x-vf-trace-tid" "SMAPI_Presence"] for event ["UICustom" "CustomButton - CustomButtonId"]
    Then Validate IOS RN Element ["x-vf-app-state" "SMAPI_Presence"] for event ["UICustom" "CustomButton - CustomButtonId"]
    Then Validate IOS RN Element ["x-vf-trace-transaction-id" "SMAPI_Presence"] for event ["UICustom" "CustomButton - CustomButtonId"]
    Then Validate IOS RN Element ["x-vf-net-type" "SMAPI_Presence"] for event ["UICustom" "CustomButton - CustomButtonId"]
    Then Validate IOS RN Element ["x-vf-net-band" "SMAPI_Presence"] for event ["UICustom" "CustomButton - CustomButtonId"]

#//////////////////////////////// Navigate from UI Custom to Main Page

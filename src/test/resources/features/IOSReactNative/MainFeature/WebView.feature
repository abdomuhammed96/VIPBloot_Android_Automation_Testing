Feature: check SMAPI WebView

  Background: User Opens App and Capture Events
    When Capture IOS RN Event
    When Click On IOS RN Button ["Open WebView"]
    When Capture IOS RN Event

  Scenario: Check WebView 1st events
    Then Validate IOS RN Element ["page-name" "WebView"] for event ["Page" "Load Time"]
  ...
  Scenario: Check WebView 2nd events
  ...
  Scenario: Check WebView 3rd events
    Then Validate IOS RN Element ["page-name" "WebView"] for event ["Page" "NA"]
...
#//////////////////////////////// Navigate from WebView to Main Page

  Scenario: Check Navigate from WebView to Main Page 1st events
    When Click On IOS RN Button ["Main, back"]
    When Capture IOS RN Event
    Then Validate IOS RN Element ["page-name" "WebView"] for event ["Page" "Stay Time"]
    Then Validate IOS RN Element ["subpage-name" "NA"] for event ["Page" "Stay Time"]
    Then Validate IOS RN Element ["event-description" "SMAPI_Presence"] for event ["Page" "Stay Time"]
    Then Validate IOS RN Element ["x-vf-trace-timestamp" "SMAPI_Presence"] for event ["Page" "Stay Time"]
    Then Validate IOS RN Element ["device-orientation" "SMAPI_Presence"] for event ["Page" "Stay Time"]
  ...
  Scenario: Check Navigate from WebView to Main Page 2nd events
    When Click On IOS RN Button ["Main, back"]
    When Capture IOS RN Event
  ...
  Scenario: Check Navigate from WebView to Main Page 3rd events
    When Click On IOS RN Button ["Main, back"]
    When Capture IOS RN Event
    Then Validate IOS RN Element ["page-name" "WebView"] for event ["Page" "NA"]
    Then Validate IOS RN Element ["subpage-name" "NA"] for event ["Page" "NA"]

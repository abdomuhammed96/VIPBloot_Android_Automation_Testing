Feature: check SMAPI Verbose

  Background: User Opens App and Capture Events
    When Capture IOS RN Event

  Scenario: Check Verbose ON OFF
    When Click On IOS RN Button ["isVerbose"]
    When Capture IOS RN Event
  ...
  Scenario: Check no events Verbose OFF
    When Click On IOS RN Button ["isVerbose"]
    When Capture IOS RN Event
  ...
  Scenario: Check events captures when Verbose OFF
    When Click On IOS RN Button ["isVerbose"]
    When Capture IOS RN Event
    When Click On IOS RN Button ["Start Timer with ID: 12345678 with element name: timerTestElement"]
    When Capture IOS RN Event
